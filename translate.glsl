...
void main() {
  ...
  // 십자가의 위치를 실제로 이동시키는 방법(1번 방법)
  // 십자가의 loc 위치값이 원 궤도를 그리게 하려면, loc에 원의 좌표값을 넣어주면 됨.
  // 원의 좌표값을 계산하는 공식 알고 있지?
  // x = sin(각도) * 반지름, y = cos(각도) * 반지름
  // 이때 각도값을 u_time으로 줘서 각도값이 매 프레임마다 변하게 해주는거임.
  // 이렇게 하면 반지름 rad에 해당하는 원의 좌표값이 매 프레임마다 각도를 달리하여 loc에 할당되므로
  // 십자가의 위치가 원의 좌표값을 갱신하면서 원 궤도를 따라 이동하게 되어있음.
  float rad = 0.5;
  vec2 loc = vec2(0.) + vec2(sin(u_time) * rad, cos(u_time) * rad);

  vec3 col = vec3(cross(loc, vec2(0.3, 0.02), coord));
